...
close. This rollout is deliberately one phase, not the usual 3–5.
...
Risks #2–#4 are real and sourced, but are **explicitly deferred** per the
author's Q1/Q5 scope override — see §7. Only Risk #1 is scheduled in §3.
This is a prioritization decision, not padding: Risk #1 is the only
High-impact row and the only one with zero end-to-end coverage.
...
|---|------------|------------------|----------------|-------------|--------|----------------|
| 1 | Critical-path persistence coverage | Prove the create/open → pin task → save → reopen flow preserves the task and its exact marker position end-to-end | #1 | integration | change opened | context/changes/testing-critical-path-persistence/ |

**Status vocabulary** (fixed): `not started` → `change opened` →
`researched` → `planned` → `implementing` → `complete`.

This rollout intentionally has one phase. Risks #2–#4 are recorded in §2
for traceability but have no phase; re-run `/10x-test-plan --refresh` if
the author's scope constraint changes and a wider rollout becomes
warranted.
